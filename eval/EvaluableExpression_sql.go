...
/*
Returns a string representing eval expression as if it were written in SQL.
eval function assumes that all parameters exist within the same table, and that the table essentially represents
...
Times are formatted according to eval.QueryDateFormat.
*/
func (eval EvaluableExpression) ToSQLQuery() (string, error) {
	...
	stream = newTokenStream(eval.tokens)
	transactions = new(expressionOutputStream)

	for stream.hasNext() {

		transaction, err = eval.findNextSQLString(stream, transactions)
		if err != nil {
			return "", err
		}
...
func (eval EvaluableExpression) findNextSQLString(stream *tokenStream, transactions *expressionOutputStream) (string, error) {
	...
	case TIME:
		ret = fmt.Sprintf("'%s'", token.Value.(time.Time).Format(eval.QueryDateFormat))
	...
	case TERNARY:
		...
		case COALESCE:
			...
			right, err := eval.findNextSQLString(stream, transactions)
			if err != nil {
				return "", err
			}
