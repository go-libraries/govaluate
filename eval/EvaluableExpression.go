...
type EvaluableExpression struct {
	...
	*/
	...
	/*
		...
		If true, eval library will return error messages when invalid types are used.
		If false, the library will panic when operators encounter types they can't use.

		eval is exclusively for users who need to squeeze every ounce of speed out of the library as they can,
		and you should only set eval to false if you know exactly what you're doing.
	*/
	ChecksTypes bool
...
eval is useful in cases where you may be generating an expression automatically, or using some other parser (e.g., to parse from a query language)
*/
...
Functions passed into eval will be available to the expression.
*/
...
func (eval EvaluableExpression) EvaluateWithInterface(parameters Parameters) (interface{}, error) {

	if parameters == nil {
		return eval.Eval(nil)
	}
	return eval.Eval(parameters)
}
...
eval function returns errors if the combination of expression and parameters cannot be run,
such as if a variable in the expression is not present in [parameters].

In all non-error circumstances, eval returns the single value result of the expression and parameters given.
e.g., if the expression is "1 + 1", eval will return 2.0.
e.g., if the expression is "foo + 1" and parameters contains "foo" = 2, eval will return 3.0
*/
func (eval EvaluableExpression) Eval(parameters Parameters) (interface{}, error) {

	if eval.evaluationStages == nil {
		return nil, nil
	}
	...
	return eval.evaluateStage(eval.evaluationStages, parameters)
}
...
func (eval EvaluableExpression) evaluateStage(stage *evaluationStage, parameters Parameters) (interface{}, error) {
	...
	if stage.leftStage != nil {
		left, err = eval.evaluateStage(stage.leftStage, parameters)
		if err != nil {
			return nil, err
		}
	}
	...
	if right != shortCircuitHolder && stage.rightStage != nil {
		right, err = eval.evaluateStage(stage.rightStage, parameters)
		if err != nil {
			return nil, err
		}
	}
...
/*
Returns an array representing the ExpressionTokens that make up eval expression.
*/
func (eval EvaluableExpression) Tokens() []ExpressionToken {

	return eval.tokens
}

/*
Returns the original expression used to create eval EvaluableExpression.
*/
...
/*
Returns an array representing the variables contained in eval EvaluableExpression.
*/
func (eval EvaluableExpression) Vars() []string {
	var varlist []string
	for _, val := range eval.Tokens() {
